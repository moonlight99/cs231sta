...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        '''
        sample_mean=np.mean(x,axis=0) #beta
...
def batchnorm_backward_test(dout,cache):
...
def batchnorm_backward(dout, cache):
    ...
    # step 8:
    dgamma=np.sum(dgammax*cache['x_hat'],axis=0)
    dx_hat=cache['gamma']*dgammax

    # step 7:
    dxmu1=dx_hat*cache['ivar']
    divar=np.sum(dx_hat*cache['xmu'],axis=0)

    #step 6:
    dsqrtvar=divar*(-1.)/(cache['sqrtvar']**2)

    #step 5:
    dvar=0.5*dsqrtvar/dsqrtvar
...
def batchnorm_backward_alt(dout, cache):
    ...
    #step_alternative
    dmu=dx_hat*(cache['x_hat'])**3 * (cache['xmu']/(cache['sqrtvar']**2) - ( cache['xmu']**2 *1./N *np.sum(cache['xmu'],axis=0) /cache['sqrtvar']**4 ) )
    print 'Alternative dxmu = ',dmu

    # step 1:
    dx1=(-1)*np.sum(dmu,axis=0)
    dx2 = 1. / N * (np.ones((N, D)) * dmu)
    dx=dx1+dx2
    # step 0:

    print 'dx in alternativ = ',dx
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
